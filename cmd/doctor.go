...
func newDoctorCommand() *cli.Command {
	return &cli.Command{
		...
		Description: "A command to diagnose problems with the current Forgente instance according to the given configuration. Some problems can optionally be fixed by modifying the database or data storage.",
...
func newRecreateTableCommand() *cli.Command {
	return &cli.Command{
		...
		Description: `The database definitions Forgente uses change across versions, sometimes changing default values and leaving old unused columns.
