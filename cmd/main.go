...
// * ./forgente -c /dev/null -h
// * ./forgente -c help /dev/null help
// * ./forgente help -c /dev/null
// * ./forgente help -c /dev/null web
// * ./forgente help web -c /dev/null
// * ./forgente web help -c /dev/null
// * ./forgente web -h -c /dev/null
...
func NewMainApp(appVer AppVersion) *cli.Command {
	app := &cli.Command{}
	app.Name = "forgente" // must be lower-cased because it appears in the "USAGE" section like "forgente doctor [command [command options]]"
	app.Usage = "A painless self-hosted Git service"
	app.Description = `Forgente program contains "web" and other subcommands. If no subcommand is given, it starts the web server by default. Use "web" subcommand for more web server arguments, use other subcommands for other purposes.`
	...
	app.Flags = []cli.Flag{
		&cli.StringFlag{
			...
			Aliases:   []string{"w"},
			TakesFile: true,
			Usage:     "Set Forgente's working path (defaults to the Forgente's binary directory)",
		},
		&cli.StringFlag{
			Name:      "config",
			Aliases:   []string{"c"},
