...
func batchProcess[T any](x base.EngineMigration, buf []T, query func(limit, start int) base.Session, process func(base.Session, T) error) error {
	size := cap(buf)
	start := 0
	for {
...
func AddHeaderAuthorizationEncryptedColWebhook(x base.EngineMigration) error {
	...
	err = batchProcess(x,
		make([]*Webhook, 0, 50),
		func(limit, start int) base.Session {
			return x.Where("type=?", "matrix").OrderBy("id").Limit(limit, start)
		},
		func(sess base.Session, hook *Webhook) error {
	...
	err = batchProcess(x,
		...
		func(sess base.Session, hookTask *HookTask) error {
