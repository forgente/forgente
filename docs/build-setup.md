...
This document lists the tools you need to build Forgente from source and how to get
...
Forgente manages frontend dependencies with [pnpm](https://pnpm.io/). The `make`
targets invoke it for you, so installing pnpm manually is only needed if you want
to run `pnpm` commands directly.

### Make

Forgente uses [Make](https://www.gnu.org/software/make/) to drive builds, linting, and
...
Linting the templates, workflow files, and YAML requires Python tooling that Forgente
...
```bash
git clone https://github.com/forgente/forgente
```

To contribute changes, [fork the repository](https://github.com/forgente/forgente) on
