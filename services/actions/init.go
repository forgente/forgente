...
func initGlobalRunnerToken(ctx context.Context) error {
	...
	token, _ := setting.EnvWithFallback("FORGENTE_RUNNER_REGISTRATION_TOKEN", "GITEA_RUNNER_REGISTRATION_TOKEN")
	tokenFile, _ := setting.EnvWithFallback("FORGENTE_RUNNER_REGISTRATION_TOKEN_FILE", "GITEA_RUNNER_REGISTRATION_TOKEN_FILE")
	if token != "" && tokenFile != "" {
		return errors.New("both FORGENTE_RUNNER_REGISTRATION_TOKEN and FORGENTE_RUNNER_REGISTRATION_TOKEN_FILE are set, only one can be used")
	}
	if tokenFile != "" {
		file, err := os.ReadFile(tokenFile)
		if err != nil {
			return fmt.Errorf("unable to read FORGENTE_RUNNER_REGISTRATION_TOKEN_FILE: %w", err)
		}
	...
	if len(token) < 32 {
		return errors.New("FORGENTE_RUNNER_REGISTRATION_TOKEN must be at least 32 random characters")
	}
	...
	if existing != nil {
		if !existing.IsActive {
			log.Warn("The token defined by FORGENTE_RUNNER_REGISTRATION_TOKEN is already invalidated, please use the latest one from web UI")
		}
		return nil
	}
