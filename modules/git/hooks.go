...
func getHookTemplates() (hookNames, hookTpls, forgenteHookTpls []string) {
	hookNames = []string{"pre-receive", "update", "post-receive"}
	hookTpls = []string{
		// for pre-receive
		fmt.Sprintf(`#!/usr/bin/env %s
# AUTO GENERATED BY FORGENTE, DO NOT MODIFY
data=$(cat)
exitcodes=""
hookname=$(basename $0)
...
`, setting.ScriptType, util.ShellEscape(setting.AppPath), util.ShellEscape(setting.CustomConf)))
	forgenteHookTpls = append(forgenteHookTpls, "")

	return hookNames, hookTpls, forgenteHookTpls
}
...
func createDelegateHooks(hookDir string) (err error) {
	hookNames, hookTpls, forgenteHookTpls := getHookTemplates()

	for i, hookName := range hookNames {
		oldHookPath := filepath.Join(hookDir, hookName)
		newHookPath := filepath.Join(hookDir, hookName+".d", "forgente")
		// pre-hard-fork delegate name; removed on regeneration so the skeleton hook can't run both
		legacyHookPath := filepath.Join(hookDir, hookName+".d", "gitea")
		...
		if err = os.WriteFile(newHookPath, []byte(forgenteHookTpls[i]), 0o777); err != nil {
...
func checkDelegateHooks(hookDir string) ([]string, error) {
	...
	for i, hookName := range hookNames {
		...
		if string(contents) != forgenteHookTpls[i] {
