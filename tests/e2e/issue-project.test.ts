...
test('assign issue to project and change column', async ({page}) => {
  ...
  const user = env.FORGENTE_TEST_E2E_USER;
...
test('create a project', async ({page}) => {
  ...
  try {
    ...
    await page.goto(`/${env.FORGENTE_TEST_E2E_USER}/${repoName}/projects/new`);
    ...
    await expect(page).toHaveURL(new RegExp(`/${env.FORGENTE_TEST_E2E_USER}/${repoName}/projects$`));

    // Verify the project appears in the list
    await expect(page.locator('.milestone-list')).toContainText(projectTitle);
  } finally {
    await apiDeleteRepo(page.request, env.FORGENTE_TEST_E2E_USER, repoName);
  }
});
...
test('assign issue to multiple projects via sidebar', async ({page}) => {
  ...
  try {
    ...
    const project1 = await createProject(page, {
      owner: env.FORGENTE_TEST_E2E_USER,
      repo: repoName,
      title: project1Title,
    });
    ...
    await page.goto(`/${env.FORGENTE_TEST_E2E_USER}/${repoName}/issues/${issue.index}`);
...
test('create issue with multiple projects pre-selected', async ({page}) => {
  ...
  try {
    ...
    await page.goto(`/${env.FORGENTE_TEST_E2E_USER}/${repoName}/issues/new`);
    ...
    await page.waitForURL(new RegExp(`/${env.FORGENTE_TEST_E2E_USER}/${repoName}/issues/\\d+`));
...
test('filter issues by multiple projects in issue list', async ({page}) => {
  ...
  try {
    ...
    await page.goto(`/${env.FORGENTE_TEST_E2E_USER}/${repoName}/issues?project=${project1.id}`);
    ...
    await page.goto(`/${env.FORGENTE_TEST_E2E_USER}/${repoName}/issues?project=${project2.id}`);
...
test('filter issues with no project using project=-1', async ({page}) => {
  ...
  try {
    ...
    await page.goto(`/${env.FORGENTE_TEST_E2E_USER}/${repoName}/issues?type=all&state=open`);
    ...
    await page.goto(`/${env.FORGENTE_TEST_E2E_USER}/${repoName}/issues?type=all&state=open&project=-1`);
...
test('close project and view in closed projects list', async ({page}) => {
  ...
  try {
    ...
    await page.goto(`/${env.FORGENTE_TEST_E2E_USER}/${repoName}/projects`);
    ...
    await page.waitForURL(new RegExp(`/${env.FORGENTE_TEST_E2E_USER}/${repoName}/projects/${projectToClose.id}`));
