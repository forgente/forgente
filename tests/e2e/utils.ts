...
export const timeoutFactor = Number(env.FORGENTE_TEST_E2E_TIMEOUT_FACTOR) || 1;

export function baseUrl() {
  return env.FORGENTE_TEST_E2E_URL?.replace(/\/$/g, '');
}
...
export function apiHeaders() {
  return apiAuthHeader(env.FORGENTE_TEST_E2E_USER, env.FORGENTE_TEST_E2E_PASSWORD);
}
...
export async function apiCreateUser(requestContext: APIRequestContext, username: string) {
  await apiRetry(() => requestContext.post(`${baseUrl()}/api/v1/admin/users`, {
    headers: apiHeaders(),
    data: {username, password: testUserPassword, email: `${username}@${env.FORGENTE_TEST_E2E_DOMAIN}`, must_change_password: false},
  }), 'apiCreateUser');
}
...
export async function login(page: Page, username = env.FORGENTE_TEST_E2E_USER, password = env.FORGENTE_TEST_E2E_PASSWORD) {
